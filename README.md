...
Для использования программы необходимо подключение к сети Интернет и достаточно одного пользователя. В его полномочия входит: получение информации о наличии свободных мест и времени в данном хостеле, а также бронирование любого доступного в данный момент жилья

в необходимое ему время. По запросу пользователь может отменить свою бронь, после чего она снова становится доступной для бронирования.
...
Информация представляется в виде реляционной базы данных в таблицах.

12.3. Требования по применению СУБД :

База данных необходима для хранения информации о гостях и номерах, а также о состоянии номеров и получение, и обработки запросов пользователей. Система будет обеспечивать целостность информации.
...
База данных по возможности заполняется на русском языке, она должна иметь полную и непротиворечивую информацию о всех номерах и времени.

13.2. Требования к источникам информации:

Источниками информации являются владельцы хостела и пользователи непосредственно. Данные берутся со стороны клиента и отправляются на сервер в базу данных. Вся информация обязана быть достоверной и переданной добровольно.
